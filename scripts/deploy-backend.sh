...
#      ANTHROPIC_MODEL, IMAGE_SCENE_MODEL, IMAGE_CHECK_MODEL, FAL_KEY, FAL_MODEL
#      are present in the environment (missing ones are skipped, existing
...
#   ANTHROPIC_API_KEY, ANTHROPIC_MODEL, IMAGE_SCENE_MODEL, IMAGE_CHECK_MODEL,
#   FAL_KEY, FAL_MODEL
#
set -euo pipefail
...
secret_args=()
secret_names=""
for name in ANTHROPIC_API_KEY ANTHROPIC_MODEL IMAGE_SCENE_MODEL IMAGE_CHECK_MODEL FAL_KEY FAL_MODEL; do
  value="${!name:-}"
  if [ -n "$value" ]; then
