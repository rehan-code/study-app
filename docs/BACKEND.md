...
| Storage 403 / `new row violates row-level security` | The request reached storage without an authenticated user. Make sure the JWT is a signed-in user token (not the anon key) and the migration in `supabase/migrations/0001_init.sql` has been pushed (it creates the buckets and policies). |
