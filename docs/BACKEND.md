...
| ------------------- | -------- | --------------------------------------------------------- |
| `ANTHROPIC_API_KEY` | yes      | page parsing, and the card-image scene writer and check   |
| `ANTHROPIC_MODEL`   | no       | override parsing model, default `claude-sonnet-5`         |
| `IMAGE_SCENE_MODEL` | no       | override scene model, default `claude-haiku-4-5-20251001` |
...
| `{"error":"Couldn't make a clean image..."}`        | Four generations in a row broke a card-image rule (writing, a face with features, an uncovered woman), so none was saved. Often a meaning about writing ("the letter", "he writes"). Retry, or reword the meaning more concretely. Logs show each discarded attempt with its scene and what the checker saw.     |
| Card images show text, faces or uncovered women     | The image check could not run, so images went out unverified. Edge Functions -> generate-card-image -> Logs will say `skipping image check` (no `ANTHROPIC_API_KEY`) or `image check rejected` (bad key, rate limit). Fix the key and regenerate the image from the card screen.                                 |
...
| Book import says the PDF is over the upload limit   | Storage rejects uploads above the project's global limit (50 MB by default). Raise it at Dashboard -> Storage -> Settings -> "Upload file size limit"; the Free plan caps it at 50 MB, so a bigger curriculum PDF needs a paid plan or a split PDF. The buckets themselves set no `file_size_limit`.             |
