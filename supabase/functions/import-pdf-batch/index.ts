...
function groupIntoLines(items: PositionedText[]): PageLine[] {
...
async function extractPositionedPages(
  ...
  for (let pageNumber = fromPage; pageNumber <= lastPage; pageNumber += 1) {
    ...
    // Must run on the drawing order pdf.js returns, before any sorting or
    // normalization: that order is what says which word a detached haraka
    // belongs to, and it reads the presentation-form marks as printed.
    const items = reattachDisplacedMarks(drawn)
      .map((item) => ({ ...item, text: normalizeArabic(item.text) }))
